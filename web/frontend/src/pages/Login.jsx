    ...
    <div className="min-h-[calc(100vh-64px)] flex items-center justify-center px-4 py-12">
      <motion.div initial={{ opacity: 0, y: 20 }} animate={{ opacity: 1, y: 0 }} className="w-full max-w-md">
        ...
        <div className="text-center mb-8">
          ...
          <p className="text-gray-400">Sign in to continue to FitFact</p>
        </div>

        {verified && (
          <div className="mb-6 p-4 bg-green-500/10 border border-green-500/30 rounded-xl">
            <p className="text-green-400 text-sm text-center">✓ Email verified successfully! You can now sign in.</p>
          </div>
        )}
        ...
        {(error || authError) && (
          <div className="mb-6 p-4 bg-red-500/10 border border-red-500/30 rounded-xl flex items-start gap-3">
            ...
            <div>
              ...
              {needsVerification && (
                <p className="text-accent-orange text-sm mt-1">Please check your email for a verification link.</p>
              )}
            </div>
          </div>
        )}
          ...
          <form onSubmit={handleSubmit} className="space-y-4">
            ...
            <div>
              <div className="flex justify-between items-center mb-1.5">
                ...
                <Link to="/forgot-password" className="text-xs text-accent-orange hover:underline">Forgot password?</Link>
              </div>
              <div className="relative">
            ...
            <button type="submit" disabled={loading} className="w-full py-3 bg-gradient-to-r from-accent-orange to-accent-gold text-white font-medium rounded-xl hover:opacity-90 transition disabled:opacity-50 flex items-center justify-center gap-2">
        ...
        <p className="text-center mt-6 text-gray-400">
          Don't have an account? <Link to="/register" className="text-accent-orange hover:underline font-medium">Sign up</Link>
        </p>
      </motion.div>
    </div>
  )
